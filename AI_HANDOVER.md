...
5. **Backblaze B2 Setup & Backend Image Optimization**: Provisioned Backblaze B2 for heavy file sharing. The backend `uploads.controller.ts` automatically resizes images and converts them to `.webp` using `sharp` before uploading to B2, bypassing Vercel's expensive optimization.
