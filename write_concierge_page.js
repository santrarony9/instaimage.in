...
file = file.replace('  return (', mockData);
...
file = file.replace(/<p className="text-base md:text-lg text-gray-300 max-w-2xl mx-auto mb-8">[\s\S]*?<HeroSearchBar \/>[\s\S]*?<\/div>[\s\S]*?<\/div>/, heroReplace);

file = file.replace(/<div className="max-w-\[1400px\] mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-30 mb-10\">\s*<div className="flex overflow-x-auto gap-4 snap-x snap-mandatory pb-4 scrollbar-hide" style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>/, '<div className="w-full relative z-30 mb-10 overflow-hidden">\n        <div className="offer-marquee-container flex w-max hover:[animation-play-state:paused]">\n          {[...Array(2)].map((_, idx) => (\n            <div key={idx} className="flex gap-4 pr-4 pl-4 sm:pl-0">');
file = file.replace(/<style dangerouslySetInnerHTML=[\s\S]*?<\/div>/, `<style dangerouslySetInnerHTML={{__html: \`
...
file = file.replaceAll('shrink-0 snap-center', 'shrink-0');
